...
export const generateCustomers = (): Customer[] => {
  ...
  return Array.from(Array(900).keys()).map((id) => {
    const first_seen = randomDate() || new Date()
    const last_seen = randomDate(first_seen) || new Date()
    ...
    const birthday = has_ordered ? faker.date.past({ years: 60 }) : null
    const avatar = has_ordered ? `https://marmelab.com/posters/avatar-${numberOfCustomers}.jpeg` : undefined
    ...
    return {
      ...
      stateAbbr: has_ordered ? faker.location.state({ abbreviated: true }) : null,
      avatar,
